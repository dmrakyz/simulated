 ...
 *   granular  — fluid EOS + no tension + internal friction (sand, mud)
 *   elastic   — fixed-corotated (neo-Hookean-ish) via polar decomposition (snow, ice)
 *   light gas — fluid EOS + Archimedes buoyancy correction (air)
 *
...
export const MATERIALS = [
  ...
  // id 1: Sand — granular, piles at angle of repose
  ...
  // id 7: Ice — stiff elastic, holds shape and bounces
  { name:'Ice',   col:0xaadcff, rho:917,  E:8.0e5, nu:0.32, type:'elastic', cScale:1.00, vdamp:1.000 },
  // id 8: Air — rho=40 (real≈1.2), boosted for numerical stability; buoyancy applied in G2P
...
/* Polar decomposition scratch buffer — rotation factor R (row-major 3×3). */
const _R = new Float64Array(9);

/**
 * Polar decomposition F = R·S  →  writes orthogonal factor R into _R.
 * Iterates  R ← ½(R + R^{-T})  (Higham's Newton iteration).
 */
function polarR(F, o) {
...
export class MPM {
  constructor(opts = {}) {
    ...
    this.DINV   = 4.0 * this.INV * this.INV;
  ...
  _step(DT) {
    ...
    for (let p = 0; p < nP; p++) {
      ...
      if (type === 2) { /* elastic: fixed-corotated Kirchhoff stress */
        ...
        // Kirchhoff stress component: P = M · Fᵀ  (row-major: P[i,j]=Σ_k M[i,k]·F[j,k])
        ...
        // τ = 2μ·(F−R)·Fᵀ + λ·J·(J−1)·I
        const mu2 = 2.0 * _mMu[mt];
        ...
        /* fluid / granular: isotropic pressure from volume ratio J.
           One-sided: only compression (J<1) generates pressure. */
        const J = pJ[p];
    ...
    /* ── G2P (grid → particle) + deformation update + advect ── */
    ...
    for (let p = 0; p < nP; p++) {
      ...
      const xp = px[p], yp = py[p], zp = pz[p];
      const bx = (xp*INV-0.5)|0, by = (yp*INV-0.5)|0, bz = (zp*INV-0.5)|0;
      const fx = xp*INV-bx, fy = yp*INV-by, fz = zp*INV-bz;
      ...
      /* Archimedes buoyancy for very light fluids (air, rho≈40).
         Samples grid mass at the centre cell of the 3×3×3 stencil.
         Quadratic B-spline centre-cell average weight ≈ 0.30, so:
           rho_local ≈ gM[centre] / (0.30 * PVOL)
         When surrounded by denser fluid this is >> rho_air, giving
         a net upward acceleration that makes air bubble through water. */
      if (type === 0 && _mRho[mt] < 100) {
        ...
        if (cx>=0&&cx<N&&cy>=0&&cy<N&&cz>=0&&cz<N) {
          ...
          if (rho_local > 300) {
            // net upward = (ρ_fluid/ρ_air − 1) × |g|, scaled 0.15 for stability
      ...
      } else { /* fluid / granular: J ← J·(1 + dt·tr(C)) */
        let J = pJ[p] * (1.0 + DT*(C0+C4+C8));
        if (type === 1 && J > 1.0) J = 1.0;

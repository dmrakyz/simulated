 ...
 *
 * Prefers running the MLS-MPM solver in a Web Worker (off the main thread, so
 * the UI never freezes). If a module worker can't be created it falls back
 * transparently to running the engine on the main thread.
 *
 * The render side only ever reads `controller.count` + `controller.snapshot`
 * (a Float32Array, stride 4: x, y, z, matId), so it doesn't care which backend
 * is active. Call `stepLocal()` once per frame — it's a no-op under the worker.
 */
...
export class SimController {
  ...
  /* ── Main-thread fallback stepping (no-op under the worker) ─── */
  stepLocal() {
